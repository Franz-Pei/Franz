...
# 配置变量
APP_DIR="/www/wwwroot/backend"
VENV_PATH="$APP_DIR/venv"
APP_SCRIPT="$APP_DIR/main.py"
LOG_FILE="/home/ubuntu/flask_monitor.log"
PID_FILE="/home/ubuntu/flask_app.pid"
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        if ps -p "$PID" > /dev/null; then
            ...
            if ps -p "$PID" -o command= | grep -q "python.*main.py"; then
                PORT_LISTENING=$(netstat -tlnp 2>/dev/null | grep -E "127.0.0.1:5000.*$PID/python" | wc -l)
...
start_app() {
    ...
    # 启动应用并将输出重定向到日志文件
    nohup python "$APP_SCRIPT" > /home/ubuntu/flask_app.log 2>&1 &
